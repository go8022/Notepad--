...
- **EXE 변환 가능** — PyInstaller 폴더형 빌드 후 zip 배포 가능
...
빌드 결과: `release/Notepad--_v1.5_win64.zip` → 이 zip 파일을 전달하면 됩니다.
...
pyinstaller --windowed --name "Notepad--" phil_notepad_plus_1.5.py
```

### 아이콘 포함 빌드

```bash
pyinstaller --windowed --icon=PhilNotepadPlus.ico --name "Notepad--" --add-data "welcome.txt;." phil_notepad_plus_1.5.py
```
...
프로젝트 폴더/
├── dist/
│   └── Notepad--/                 ← zip으로 묶을 폴더형 실행 파일
│       ├── Notepad--.exe
│       └── ...
├── build/                     ← 삭제 가능
├── release/
│   └── Notepad--_v1.5_win64.zip  ← 배포할 파일
└── Notepad--.spec             ← 삭제 가능
```
...
| 항목 | 내용 |
|------|------|
| 파일 크기 | zip 기준 약 8~15 MB (tkinter만 사용하므로 경량) |
